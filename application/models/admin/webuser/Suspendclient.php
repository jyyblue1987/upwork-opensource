<?php
class Suspendclient extends CI_Model {
    ...
    function __construct() {
	  ...
	  function check($permission) {
		...
		if($this->Adminlogincheck->checkper($permission['suspendclient'])){
			return true;
		}else{
			redirect(site_url());
			exit();
	  ...
	  function load($permission,$mod){
		...
		$title=$this->Adminforms->getdata("name","usersubpage",$permission['suspendclient']);
