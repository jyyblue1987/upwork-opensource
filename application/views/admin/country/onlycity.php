  ...
  <div class="page animsition">
      ...
      <div class="row">
        <div class="col-md-6">
          ...
          <div class="panel">
            ...
            <form class="form-horizontal" id="exampleStandardForm" method="post" action="<?php echo site_url('administrator/userpage/loadpage/'.$loadpage.'/subpage/addcity/?id='.$_GET['id']); ?>"  enctype="multipart/form-data" autocomplete="off">
              <?php
	...
	echo $this->Adminforms->inputtext(ucfirst($val),$db."_".$val,"Enter ".ucfirst($val),"",1);
			  ...
			  echo $this->Adminforms->submit("Add City");
    ...
    <div class="page-header">
      <h1 class="page-title">Active city</h1>
   ...
   if($activecity['showing'] > 0)
   {
		   
foreach ($activecity['data'] as  $value) {
	$arrdata=(array) $value;
	  ?>
				  <tr data-index="0">
				  ...
				  <div style="float:left;"><b><?php echo $arrdata[$qdata['db']."_name"]; ?></b></div>
				    ...
				    <div class="pull-right search">
						  <a href="<?php echo site_url('administrator/userpage/loadpage/'.$loadpage.'/subpage/deletecity?country='.$_GET['id'].'&id='.$arrdata[$qdata['db']."_id"]); ?>">
                          <button type="button" class="btn btn-danger" data-toggle="tooltip"
                          data-original-title="Delete City">Delete</button>
                          </a>
					</div>
				  
				   <form method="POST" action="<?php echo site_url('administrator/userpage/loadpage/'.$loadpage.'/subpage/editcity?country='.$_GET['id'].'&id='.$arrdata[$qdata['db']."_id"]); ?>" id="search">
    ...
    <div class="page-header">
      <h1 class="page-title">Inactive city</h1>
   ...
   if($inactivecity['showing'] > 0)
   {
		   
foreach ($inactivecity['data'] as  $value) {
	$arrdata=(array) $value;
	  ?>
				  <tr data-index="0">
				  ...
				  <td style="">
						  ...
						  <a href="<?php echo site_url('administrator/userpage/loadpage/'.$loadpage.'/subpage/restorecity?country='.$_GET['id'].'&id='.$arrdata[$qdata['db']."_id"]); ?>">
                          <button type="button" class="btn btn-success" data-toggle="tooltip"
                          data-original-title="Restore City">Restore</button>
                          </a>
						  <a href="<?php echo site_url('administrator/userpage/loadpage/'.$loadpage.'/subpage/delcity?country='.$_GET['id'].'&id='.$arrdata[$qdata['db']."_id"]); ?>">
                          <button type="button" class="btn btn-danger" data-toggle="tooltip"
                          data-original-title="Delete City">Delete</button>
                          </a>
